...
interface FormData {
  ...
  paymentMethod:   'cash' | 'check' | 'credit' | 'zelle' | '';
  signature:       string;
  date:            string;
...
const Enrollment = ({ onNavigate }: EnrollmentProps) => {
  ...
  const [formData, setFormData] = useState<FormData>({
    ...
    selectedCourses: [], wantsMaterials: false, wantsShipping: false,
  ...
  const MATERIALS_FEE    = 45;
  const SHIPPING_FEE     = 15;

  const courseCount       = formData.selectedCourses.length;
  const courseSubtotal    = courseCount * COURSE_PRICE;
  const materialsSubtotal = formData.wantsMaterials ? MATERIALS_FEE : 0;
  const shippingSubtotal  = (formData.wantsShipping && formData.wantsMaterials) ? SHIPPING_FEE : 0;
  const grandTotal        = courseSubtotal + REGISTRATION_FEE + materialsSubtotal + shippingSubtotal;
      ...
      const doc      = generateEnrollmentPDF(
        {
          ...
          selectedCourses: formData.selectedCourses, wantsMaterials: formData.wantsMaterials,
          wantsShipping: formData.wantsShipping, paymentMethod: formData.paymentMethod,
          signature: formData.signature, date: formData.date,
        },
        { courseSubtotal, registrationFee: REGISTRATION_FEE, materialsSubtotal, shippingSubtotal, grandTotal }
      );
        ...
        <form onSubmit={handleSubmit} className="space-y-16 animate-fade-in">
          ...
          <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-gray-100">
            ...
            <p className="text-xs text-gray-400 mb-8 ml-11">
              Full Legal Name and Phone Number are required. All other fields are optional.
            </p>
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="md:col-span-2">
                ...
                <input
                  ...
                  className="w-full bg-gray-50 border-b border-gray-200 p-3 focus:outline-none focus:border-champagne transition-colors"
              ...
              <div>
                ...
                <input
                  ...
                  className={`w-full bg-gray-50 border-b p-3 focus:outline-none transition-colors ${
                    phoneError
              ...
              {/* Mailing Address — OPTIONAL (no required attribute) */}
              <div className="md:col-span-2">
                <label className="block text-xs uppercase tracking-widest text-gray-500 mb-2">Mailing Address</label>
                <input
                ...
                <div className="grid md:grid-cols-3 gap-6">
                  <div>
                    ...
                    <input type="text" name="caReLicense" value={formData.caReLicense} onChange={handleInputChange}
                      className="w-full bg-transparent border-b border-gray-200 py-2 text-sm focus:outline-none focus:border-champagne" />
                  </div>
                  <div>
          ...
          <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="space-y-12">
              <div>
                <h3 className="text-sm font-bold text-oxford uppercase tracking-widest mb-4 border-b border-gray-100 pb-2">Insurance Certification</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  <Checkbox label="12-Hr Ethics & CA Ins Code (Required)" subLabel="Pre-Licensing · 12h"
                    checked={formData.selectedCourses.includes('ins-ethics')} onChange={() => toggleCourse('ins-ethics')} />
                  <Checkbox label="Life & Health Insurance" subLabel="Pre-Licensing · 52h"
                    checked={formData.selectedCourses.includes('ins-life-health')} onChange={() => toggleCourse('ins-life-health')} />
                  <Checkbox label="Property & Casualty Insurance" subLabel="Pre-Licensing · 52h"
                    checked={formData.selectedCourses.includes('ins-pc')} onChange={() => toggleCourse('ins-pc')} />
                  <Checkbox label="Insurance CE Package" subLabel="Continuing Ed · Provider 409s99"
                    checked={formData.selectedCourses.includes('ins-ce')} onChange={() => toggleCourse('ins-ce')} />
                </div>
              </div>
              <div>
                <h3 className="text-sm font-bold text-oxford uppercase tracking-widest mb-4 border-b border-gray-100 pb-2">Real Estate Certification</h3>
                <div className="grid md:grid-cols-2 gap-4">
                  <Checkbox label="Real Estate Principles (Required)" subLabel="Pre-Licensing · 45h"
                    checked={formData.selectedCourses.includes('re-principles')} onChange={() => toggleCourse('re-principles')} />
                  <Checkbox label="Real Estate Practice (Required)" subLabel="Pre-Licensing · 45h"
                    checked={formData.selectedCourses.includes('re-practice')} onChange={() => toggleCourse('re-practice')} />
                  <Checkbox label="Real Estate Finance" subLabel="Pre-Licensing · 45h"
                    checked={formData.selectedCourses.includes('re-finance')} onChange={() => toggleCourse('re-finance')} />
                  <Checkbox label="45-Hour CE Package" subLabel="License Renewal"
                    checked={formData.selectedCourses.includes('re-ce')} onChange={() => toggleCourse('re-ce')} />
                </div>
              </div>
              <div>
                <h3 className="text-sm font-bold text-oxford uppercase tracking-widest mb-4 border-b border-gray-100 pb-2">NMLS Certification</h3>
                <div className="grid md:grid-cols-2 gap-4">
          ...
          <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="bg-gray-50 p-6 rounded-xl mb-8 border border-gray-200 space-y-3">
              <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{courseCount} selected @ ${COURSE_PRICE}/ea</p>
                </div>
              ...
              <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="optMaterials"
                    checked={formData.wantsMaterials}
                    onChange={(e) => setFormData(prev => ({
                      ...prev, wantsMaterials: e.target.checked,
                      wantsShipping: e.target.checked ? prev.wantsShipping : false,
                    }))}
                    className="w-4 h-4 accent-champagne cursor-pointer" />
                  <label htmlFor="optMaterials" className="cursor-pointer">
                    <p className="font-medium text-obsidian">Physical Materials (Optional)</p>
                    <p className="text-xs text-gray-500">Printed textbooks &amp; workbooks</p>
                  </label>
                </div>
                <p className={`font-serif text-lg transition-colors ${formData.wantsMaterials ? 'text-obsidian' : 'text-gray-300'}`}>
                  ${materialsSubtotal.toFixed(2)}
                </p>
              </div>
              <div className="flex justify-between items-center pb-3 border-b border-gray-200">
                <div className="flex items-center gap-3">
                  <input type="checkbox" id="optShipping"
                    checked={formData.wantsShipping && formData.wantsMaterials}
                    disabled={!formData.wantsMaterials}
                    onChange={(e) => setFormData(prev => ({ ...prev, wantsShipping: e.target.checked }))}
                    className="w-4 h-4 accent-champagne cursor-pointer disabled:opacity-40" />
                  <label htmlFor="optShipping" className={`cursor-pointer ${!formData.wantsMaterials ? 'opacity-40' : ''}`}>
                    <p className="font-medium text-obsidian">Shipping (Optional)</p>
                    <p className="text-xs text-gray-500">Standard Ground · 3–5 business days</p>
                  </label>
                </div>
                <p className={`font-serif text-lg transition-colors ${formData.wantsShipping && formData.wantsMaterials ? 'text-obsidian' : 'text-gray-300'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {(['Cash', 'Check', 'Credit Card', 'Zelle'] as const).map((method) => {
            ...
            {formData.paymentMethod === 'zelle' && (
              <div className="mt-6 p-4 bg-blue-50 border border-blue-100 rounded-lg text-blue-900 text-sm animate-fade-in">
                <strong>Zelle Payee:</strong> (626) 308-0150 &nbsp;—&nbsp; Please include your full name in the memo line.
              </div>
            )}
          </div>
          ...
          <div className="bg-white p-8 md:p-10 rounded-3xl shadow-sm border border-gray-100">
            ...
            <div className="flex items-start gap-4 p-4 border border-champagne/30 bg-champagne/5 rounded-xl mb-8">
              ...
              <label htmlFor="agreement" className="text-sm text-gray-700 leading-relaxed cursor-pointer">
                <strong>I acknowledge that I have read and agree to the school's refund policy, completion requirements, and terms of service.</strong> I understand I may cancel this enrollment within 7 business days of signing for a full refund minus the $35 registration fee.
              </label>
            </div>
            <div className="grid md:grid-cols-2 gap-8">
              <div>
                ...
                <input type="text" name="signature"
                  ...
                  className="w-full bg-transparent border-b border-gray-300 py-2 font-serif italic text-xl focus:outline-none focus:border-champagne"
            ...
            {submitError && (
              <div className="w-full p-4 bg-red-50 border border-red-200 rounded-xl text-sm text-red-800 animate-fade-in">
                ...
                <span className="text-xs mt-1 block">
                  ...
                  <a href="mailto:shirleymiao888@gmail.com" className="underline">shirleymiao888@gmail.com</a>
